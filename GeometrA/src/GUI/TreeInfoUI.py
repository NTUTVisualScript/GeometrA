...
from GeometrA.src.ADB.adb_roboot import ADBRobot
...
class TreeInfoUI(ttk.Treeview):
    __single = None

    def __init__(self, parent = None, *args, **kwargs):
        self.TreeInfoframe = ttk.Frame(parent)
        ttk.Treeview.__init__(self, *args, **kwargs)

        if TreeInfoUI.__single:
            raise TreeInfoUI.__single
            TreeInfoUI.__single = self

        self.Yvertscroll = Scrollbar(orient= "vertical",  command=self.yview)
        self['yscroll'] = self.Yvertscroll.set

        self["columns"] = ("zero", "one", "two")
        self['show'] = 'headings'
        self.column("zero", stretch=YES, minwidth=0, width=350)
        self.heading("zero", text="Class")
        self.column("one", width=150)
        self.heading("one", text="Text")
        self.column("two", width=150)
        self.heading("two", text="Bounding Box")

        self.grid(in_=self.TreeInfoframe, row=0, column=0, sticky=NSEW)
        self.Yvertscroll.grid(in_=self.TreeInfoframe, row=0, column=1, sticky=NS)

        self.TreeInfoframe.place(x=460, y=30)

        #self.bind("<ButtonRelease-1>", self.on_tree_select_node)

    def getTreeInfoUI(parent):
        if not TreeInfoUI.__single:
            TreeInfoUI.__single = TreeInfoUI(parent)
        return TreeInfoUI.__single
